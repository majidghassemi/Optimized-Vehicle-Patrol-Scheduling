...
class HVSP:
    ...
    def write_results_to_file(self, filename, vehicles, shifts, locations, rest_time, patrol_time, average_unique, average_total, execution_time=None):
        with open(filename, 'a') as file:  # Open in append mode
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            file.write(f"Timestamp: {timestamp}\n")
            file.write(f"{vehicles} vehicles, {shifts} shifts, {locations} locations, {rest_time} rest time, {patrol_time} patrol time => "
    ...
    def main(self):
        ...
        patrol_times = [5, 10, 15]  # Different patrol times to test
        ...
        for vehicles, locations in test_cases:
            for patrol_time in patrol_times:
                ...
                self.shifts = 6  # Fixed to 6 shifts
                self.shift_lengths = [120] * self.shifts  # Fixed shift_lengths for 6 shifts
                self.patrol_time = patrol_time
                ...
                self.write_results_to_file('AHBPS_diff_patrol.txt', self.vehicles, self.shifts, self.total_locations, self.rest_period, self.patrol_time, average_unique, average_total, execution_time)
