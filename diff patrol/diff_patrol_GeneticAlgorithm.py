...
class GeneticAlgorithm:
    def __init__(self, vehicles, locations, shifts=6, population_size=100, generations=120, rest_period=10, patrol_time=5, heuristic_file='/home/ubuntu/Optimized-Vehicle-Patrol-Scheduling/AHBPS_diff_patrol.txt'):
    ...
    def write_results_to_file(self, filename, vehicles, locations, shifts, patrol_time, best_fitness, average_fitness, execution_time=None):
        with open(filename, 'a') as file:  # Open in append mode
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            file.write(f"Timestamp: {timestamp}\n")
            file.write(f"{vehicles} vehicles, {shifts} shifts, {locations} locations, {patrol_time} patrol time => Best fitness (distinct locations visited): {best_fitness}, Average fitness: {average_fitness}\n")
...
results_filename = 'GDVPS_diff_patrol.txt'
...
# Iterate over all valid pairs and patrol times, and run the algorithm
for vehicles, locations in VALID_PAIRS:
    for patrol_time in [10, 15]:  # Different patrol times to test
        ...
        for i in range(15):
            ga = GeneticAlgorithm(vehicles=vehicles, locations=locations, shifts=shifts, patrol_time=patrol_time)
        ...
        ga.write_results_to_file(results_filename, vehicles, locations, shifts, patrol_time, best_overall_fitness, average_fitness_across_iterations, execution_time)

        # Print the details of each travel for each car in the best overall solution
        print(f"Details for {vehicles} vehicles and {locations} locations with {patrol_time} minutes patrol time:")
